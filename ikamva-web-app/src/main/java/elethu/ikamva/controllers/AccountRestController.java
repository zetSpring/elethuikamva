...
public class AccountRestController {
    ...
    @GetMapping("/{id}")
    ResponseEntity<Account> FindAccountById(
            @ApiParam(value = "An id for the account number to be retrieved.", required = true, example = "123")
                    @PathVariable
                    Long id) {
    ...
    ResponseEntity<Account> FindAccountByPrivateCompany(
            @ApiParam(
                            value = "A private company id for retrieving account details.",
                            required = true,
                            example = "000000000")
                    @PathVariable
                    Long id) {
    ...
    ResponseEntity<Account> FindAccountByAccountNo(
            @ApiParam(value = "An account no for retrieving account details", required = true, example = "0000000000")
                    @PathVariable
                    Long accountNo) {
    ...
    //    ResponseEntity<Account> updateAccount(@ApiParam(value = "Account id for an account to be updated:", required =
    // true)
    //                                          @Valid @RequestBody Account updateAccount, Long id) throws
    ...
    ResponseEntity<Account> deleteAccountByAccountNo(
            @ApiParam(value = "Account id for an account to be deleted:", required = true, example = "123")
                    @PathVariable
                    Long accountNo) {
    ...
    @ApiOperation(value = "An id for the account number to be deleted.")
    @DeleteMapping("/{id}")
    ResponseEntity<Account> deleteAccountById(
            @ApiParam(value = "Account id for an account to be deleted:", required = true, example = "123")
                    @PathVariable
                    Long id) {
