...
public class CorpCompanyServiceImpl implements CorpCompanyService {
    ...
    public CorpCompany saveCorpCompany(CorpCompany newCorpCompany) {
        if (isCorporateActive(newCorpCompany.getRegistrationNo())) {
            throw new CorpCompanyException("Corporate Company with registration number: "
                    + newCorpCompany.getRegistrationNo() + " already exists.");
        }
    ...
    public CorpCompany updateCorpCompany(CorpCompany updateCorpCompany) {
        CorpCompany updateCompany = corpCompanyRepository
                .findById(updateCorpCompany.getId())
                .orElseThrow(() -> new CorpCompanyException(
                        String.format("There is not Corporate Company found with id: %s", updateCorpCompany.getId())));
    ...
    public CorpCompany deleteCorpCompany(Long id) {
        var corpCompany = corpCompanyRepository
                .findById(id)
                .orElseThrow(
                        () -> new CorpCompanyException("Could not find corporate company id: " + id + " to delete."));
    ...
    public Boolean isCorporateActive(String companyRegistrationNo) {
        return corpCompanyRepository
                .findCorpCompanyByRegistrationNo(companyRegistrationNo)
                .isPresent();
    }
}
